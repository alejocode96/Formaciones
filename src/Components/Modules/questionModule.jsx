...
function QuestionModule({ question, answer, onContentIsEnded, onAttempt, onCorrectAnswer, isCompleted = false }) {
    ...
    const correctAnswerIndex = answer.findIndex(a => a.rsp === true);
    ...
    }, [isCompleted, correctAnswerIndex]);
    ...
    const handleSubmit = () => {
        if (selectedAnswer !== null && !isCompleted) {
            ...
            // IMPORTANTE: Incrementar el contador de intentos ANTES de validar
            if (onAttempt) {
                onAttempt();
            }
            ...
            // Si es correcta, notificar al padre y guardar progreso
            if (selectedAnswer === correctAnswerIndex) {
                // Guardar progreso inmediatamente
                if (onCorrectAnswer) {
                    onCorrectAnswer(currentAttempt);
                }
                
                // Notificar que terminó después de 2 segundos
                setTimeout(() => {
    ...
    const handleRetry = () => {
        ...
        // NO llamar a onAttempt aquí, solo al enviar respuesta
    };

    const isCorrect = selectedAnswer === correctAnswerIndex;

    return (
        <div className='max-w-5xl mx-auto pt-8 pb-14 lg:pb-4' data-aos="fade-up" data-aos-delay={300} data-aos-duration="600">
            {/* Pregunta */}
            <div className='mb-8'>
